...
4. ✅ **发布到草稿箱** - 文章发布到微信公众号草稿箱（已替代永久图文素材接口）

**重要说明**：微信公众号已于2022年2月28日下线永久图文素材接口，现在使用草稿箱接口。
...
A: 登录微信公众平台 -> 内容与互动 -> 草稿箱，找到刚发布的文章。

**注意**：永久图文素材接口已下线，现在使用草稿箱接口。

### Q: 可以自动群发吗？

A: 当前版本发布到草稿箱，需要手动在后台发布或群发。自动群发需要额外的API权限（内容发布接口）。

### Q: 为什么必须提供封面图？

A: 草稿箱接口要求必须提供封面图（thumb_media_id），这是微信API的要求。请确保在指定位置放置封面图。
